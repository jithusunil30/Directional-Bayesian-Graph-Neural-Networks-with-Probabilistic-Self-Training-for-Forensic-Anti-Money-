...
def load_ml3_data():
    """
    Loads fully-labeled PyG graph data and constructs confidence sample weights
    to down-weight ambiguous pseudo-labels during gradient updates.
    """
    base_dir = os.path.dirname(os.path.abspath(__file__))
    source_pyg_path = os.path.join(base_dir, '..', 'ML TRAINING 2', 'elliptic_pyg_data_fully_labeled.pt')
    if not os.path.exists(source_pyg_path):
        source_pyg_path = os.path.join(base_dir, '..', 'dataset', 'elliptic_pyg_data_updated.pt')

    print(f"[ML3 Data] Loading Graph Data from: {source_pyg_path}")
    data = torch.load(source_pyg_path, weights_only=False)

    x = data.x.numpy()
    y = data.y.numpy().astype(int)
    time_steps = data.time_step.numpy()

    # Load pseudo-label confidence scores if available
    csv_path = os.path.join(base_dir, '..', 'dataset', 'elliptic_dataset_fully_labeled.csv')
    if os.path.exists(csv_path):
        print(f"[ML3 Data] Incorporating soft pseudo-label confidences from: {csv_path}")
        df_labels = pd.read_csv(csv_path, usecols=['is_predicted', 'illicit_probability'])
        is_pred = df_labels['is_predicted'].values
        probs = df_labels['illicit_probability'].values
        # Confidence score: max(P, 1-P)
        conf = np.maximum(probs, 1.0 - probs)
        weights = np.where(is_pred, conf, 1.0)
    else:
        weights = np.ones(len(y), dtype=np.float32)

    data.sample_weights = torch.tensor(weights, dtype=torch.float32)

    # Temporal split masks across all 203,769 nodes
    train_mask_np = (time_steps <= 30)
    val_mask_np = (time_steps >= 31) & (time_steps <= 34)
    test_mask_np = (time_steps >= 35) & (time_steps <= 49)
    ...
    print(f"=== ML TRAINING 3 Dataset Initialized ===")
    ...
    print(f"Average Pseudo-Label Confidence Weight: {np.mean(weights):.4f}\n")
