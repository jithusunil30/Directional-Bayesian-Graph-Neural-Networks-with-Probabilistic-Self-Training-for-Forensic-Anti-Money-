...
class GCNModel(nn.Module):
    ...
    Performs spectral graph convolutions over transaction payment edges.
    """
...
class GATModel(nn.Module):
    ...
    Learns dynamic attention coefficients across payment flows.
    """
    ...
    def forward(self, x, edge_index):
        ...
        h = F.dropout(h, p=self.dropout_rate, training=self.training)
        h = F.elu(self.bn2(self.gat2(x if h is None else h, edge_index)))
        h = F.dropout(h, p=self.dropout_rate, training=self.training)
        return self.out(h).squeeze(-1)
...
class GraphSAGEModel(nn.Module):
    ...
    Inductive spatial neighborhood aggregation via mean pooling.
    """
    def __init__(self, in_features, hidden_dim=128, dropout_rate=0.3):
...
class GINModel(nn.Module):
    """
    2-Layer Graph Isomorphism Network (GIN).
    Maximally expressive spatial message passing with multi-layer perceptron aggregators.
    """
    def __init__(self, in_features, hidden_dim=128, dropout_rate=0.3):
...
# =========================================================================
# 2. Probabilistic & Bayesian Graph Neural Networks (MC Variational Inference)
# =========================================================================

class BayesianGCNModel(nn.Module):
    """
    Bayesian GCN with Monte Carlo Dropout Variational Inference.
    Decomposes uncertainty into Epistemic (Model Variance) and Aleatoric (Data Noise Entropy).
    """
    ...
    def mc_predict(self, x, edge_index, mask, num_samples=30):
        ...
        samples = np.array(samples) # (num_samples, num_test_nodes)
...
class BayesianGATModel(nn.Module):
    """
    Bayesian GAT combining Multi-Head Attention with Monte Carlo Dropout.
    Captures relational attention uncertainty across complex money laundering networks.
    """
...
class BayesianGraphSAGEModel(nn.Module):
    """
    Bayesian GraphSAGE with Monte Carlo Dropout.
    Provides inductive uncertainty estimation over dynamic scale-free graph structures.
    """
    def __init__(self, in_features, hidden_dim=128, mc_dropout_rate=0.3):
