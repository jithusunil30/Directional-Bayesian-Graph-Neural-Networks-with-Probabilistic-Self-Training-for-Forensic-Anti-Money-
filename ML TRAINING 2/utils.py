...
def load_fully_labeled_elliptic_data():
    base_dir = os.path.dirname(os.path.abspath(__file__))
    target_path = os.path.join(base_dir, 'elliptic_pyg_data_fully_labeled.pt')
    
    if not os.path.exists(target_path):
        from label_unlabeled_nodes import label_all_unlabeled_nodes
        data = label_all_unlabeled_nodes(output_dir=base_dir)
    else:
        print(f"[Data] Loading fully-labeled graph from: {target_path}")
    ...
    # Train: Steps 1 to 30
    # Val/Calibration: Steps 31 to 34
    # Test: Steps 35 to 49
    ...
    print(f"=== Fully-Labeled Dataset Loaded Successfully ===")
